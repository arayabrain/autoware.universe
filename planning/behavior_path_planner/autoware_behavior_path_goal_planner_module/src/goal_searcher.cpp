...
GoalCandidates GoalSearcher::search(const std::shared_ptr<const PlannerData> & planner_data)
{
  ...
  // create departure check lane
  const auto getNeighboringInnerLane =
    [&](const lanelet::ConstLanelet & lane) -> std::optional<lanelet::ConstLanelet> {
    const auto neighboring_inner_lane = left_side_parking_
                                          ? route_handler->getRightShoulderLanelet(lane)
                                          : route_handler->getLeftShoulderLanelet(lane);

    if (neighboring_inner_lane) return neighboring_inner_lane;
    return left_side_parking_ ? route_handler->getRightLanelet(lane)
  ...
  for (const auto & lane : pull_over_lanes) {
    if (left_side_parking_) {
      for (const auto & pt : lane.leftBound()) {
        outer_bound_points.push_back(lanelet::Point3d(pt));
      }
    } else {
      for (const auto & pt : lane.rightBound()) {
        outer_bound_points.push_back(lanelet::Point3d(pt));
      }
    }
    auto neighboring_inner_lane = getNeighboringInnerLane(lane);
    while (neighboring_inner_lane.has_value()) {
      neighboring_inner_lane = getNeighboringInnerLane(*neighboring_inner_lane);
    }
    if (neighboring_inner_lane.has_value()) {
      if (left_side_parking_) {
        for (const auto & pt : neighboring_inner_lane->rightBound()) {
          inner_bound_points.push_back(lanelet::Point3d(pt));
        }
      } else {
        for (const auto & pt : neighboring_inner_lane->leftBound()) {
          inner_bound_points.push_back(lanelet::Point3d(pt));
        }
      }
    } else {
  ...
  const auto departure_check_lane = std::invoke([&]() -> lanelet::Lanelet {
    if (left_side_parking_) {
      return lanelet::Lanelet(lanelet::utils::getId(), outer_bound_points, inner_bound_points);
    } else {
      return lanelet::Lanelet(lanelet::utils::getId(), inner_bound_points, outer_bound_points);
    }
  });
